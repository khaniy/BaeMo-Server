...
class ClubPostCommentIdTest extends FixtureMonkeyBaseSupport {
    @RepeatedTest(BaemoTestEnvironment.VALUE_OBJECT_COUNT)
    void 생성에_성공할_것이다() throws Exception {
        final var repliesId = monkey.giveMeOne(ClubPostCommentId.class);
        Assertions.assertThat(repliesId).isNotNull();
        Assertions.assertThat(repliesId.id()).isPositive();
    }
    ...
    void 잘못된_매개변수는_생성에_실패할_것이다(final Long arg) throws Exception {
        super.assertThrowInvalidArgs(() -> new ClubPostCommentId(arg));
    }
}