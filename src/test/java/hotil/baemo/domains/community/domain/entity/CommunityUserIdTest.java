...
class CommunityUserIdTest extends FixtureMonkeyBaseSupport {

    @RepeatedTest(BaemoTestEnvironment.VALUE_OBJECT_COUNT)
    void 생성에_성공할_것이다() {
        final var instance = monkey.giveMeOne(CommunityUserId.class);
        assertAll(() -> {
    ...
    void 올바르지_않은_매개변수는_생성에_실패할_것이다(final Long arg) {
        Assertions.assertThatThrownBy(() -> new CommunityUserId(arg))
