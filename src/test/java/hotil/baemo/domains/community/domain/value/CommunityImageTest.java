...
class CommunityImageTest extends FixtureMonkeyBaseSupport {

    @RepeatedTest(BaemoTestEnvironment.VALUE_OBJECT_COUNT)
    void 생성에_성공할_것이다() {
        final var instance = monkey.giveMeOne(CommunityImage.class);
        assertAll(() -> {
            Assertions.assertThat(instance).isNotNull();
            Assertions.assertThat(instance.image()).isNotBlank();
        });
    }
    ...
    void 잘못된_매개변수는_생성에_실패할_것이다(final String arg) {
        super.assertThrowInvalidArgs(() -> new CommunityImage(arg));
    }
}