...
class CommunityContentTest extends FixtureMonkeyBaseSupport {

    @RepeatedTest(BaemoTestEnvironment.VALUE_OBJECT_COUNT)
    void 생성에_성공할_것이다() {
        final var instance = monkey.giveMeOne(CommunityContent.class);
        assertAll(() -> {
    ...
    void 잘못된_매개변수는_생성에_실패할_것이다(final String arg) {
        super.assertThrowInvalidArgs(() -> new CommunityContent(arg));
    }
}