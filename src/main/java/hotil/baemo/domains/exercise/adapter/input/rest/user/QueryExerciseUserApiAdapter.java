...
import hotil.baemo.domains.exercise.application.usecases.user.query.*;
...
public class QueryExerciseUserApiAdapter {
    ...
    private final RetrieveMyGuestUseCase retrieveMyGuestUseCase;
    ...
    @Operation(summary = "내 게스트 리스트 조회")
    @GetMapping("/{exerciseId}/member/my/guest")
    public ResponseDTO<List<QExerciseUserDTO.GuestListView>> retrieveMyGuest(
        @AuthenticationPrincipal BaeMoUserDetails user,
        @PathVariable Long exerciseId
    ) {
        return ResponseDTO.ok(retrieveMyGuestUseCase.retrieveMyGuest(new UserId(user.userId()), new ExerciseId(exerciseId)));
    }
