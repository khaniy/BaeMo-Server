...
public class CommandExerciseApiAdapter {
    ...
    public ResponseDTO<Void> createExercise(
        ...
        @Parameter(description = "운동 썸네일 이미지 파일", required = false, content = @Content(mediaType = "multipart/form-data"))
        @RequestPart(value = "thumbnail", required = false) MultipartFile thumbnail
    ) {

        createExerciseUseCase.createExercise(
            new UserId(user.userId()),
            request.toVOGroup(thumbnail)
        );
    ...
    public ResponseDTO<Void> updateExerciseThumbnail(
        ...
        updateExerciseUseCase.updateThumbnail(
            new UserId(user.userId()),
            new ExerciseId(exerciseId),
            new ExerciseThumbnail(thumbnail)
        );
        return ResponseDTO.ok();
    }
